...
## M3. Discord Bot雛形
- [ ] `phase1-requirements-bot/src/discord/client.js` でBotログイン処理を実装
- [ ] `messageCreate` イベントで「対象チャンネルか」「管理者か」を判定するガード処理を実装
- [ ] 進行中セッションの有無をDBから確認する処理を実装（M2に依存）
- [ ] 上記条件を満たした投稿を受け取り、コンソールにログ出力するだけの状態を作る（AI呼び出しはまだ行わない）
...
## M4. Ollama連携
- [ ] `shared/ollama/ollamaClient.js` を実装（詳細設計書5章のリクエスト仕様に準拠）
- [ ] 役割ごとに異なるモデル名を`.env`から読み込めるようにする
- [ ] タイムアウト・リトライ処理を実装（詳細設計書6章のエラーハンドリング仕様に準拠）
- [ ] 単体で「プロンプトを投げて応答が返る」ことを確認するテストスクリプトを用意
...
## M7. 人間確認フロー
- [ ] ドラフト要約＋レビューコメントを`📝`付きでDiscordに投稿する処理を実装（`phase1-requirements-bot/src/discord/messageBuilder.js`）
- [ ] 質問（要件定義役／レビュー役由来）を指示役AIで整形し、`❓`付きでDiscordに投稿する処理を実装
- [ ] `postLongText()`等の共通関数を実装し、2000文字超の短文発言は自動分割送信、ドラフト全文は常にMarkdownファイル添付とする処理を実装
- [ ] `SHOW_AI_DISCUSSION=true`時に、指示役／要件定義役／レビュー役の発言（内部協議中含む）を`🗣️`付きでリアルタイム投稿する処理を実装
- [ ] Discordメッセージの2000文字制限に対応した分割投稿処理を実装
- [ ] 人間の返信を「OK」「修正指示」のどちらかに解釈する処理を実装（通常確認時）
- [ ] 人間の返信を質問への回答として次ラウンドの指示役入力に渡す処理を実装（質問対応時）
- [ ] 曖昧な返信の場合に再確認を促す処理を実装（両ケースとも）
- [ ] 内部協議を経て再度エスカレーションされた質問には、フォローアップである旨の一言を添える処理を実装
